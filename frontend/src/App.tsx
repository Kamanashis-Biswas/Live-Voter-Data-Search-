...
export default function App() {
      ...
      if (res.ok) {
        ...
        if (data.success && data.pdfs) {
          ...
          const mapped: UploadedPdf[] = data.pdfs.map((p: any) => ({
            ...
            uploadTime: p.uploaded_at ? new Date(p.uploaded_at).toLocaleString('bn-BD') : 'অজানা',
            district: p.district || '',
            upazila: p.upazila || '',
            unionName: p.union_name || '',
            wardNo: p.ward_no || '',
            voterArea: p.voter_area || '',
            voterAreaNo: p.voter_area_no || '',
            totalVoters: p.total_voters || 0,
            totalFemaleVoters: p.total_female_voters || 0,
            genderType: p.gender_type || 'উভয়',
            publicationDate: p.publication_date || '',
            postCode: p.post_code || '',
            voterCount: p.voter_count || 0,
            status: p.status || 'সফল',
  ...
  const handleSearch = async (filters: SearchFilters) => {
    ...
    try {
      ...
      // Map backend snake_case fields to frontend camelCase interface
      const mappedResults: VoterRecord[] = results.map((v: any) => ({
        ...
        voterAreaNo: v.voter_area_no || '',
        upazila: v.upazila || '',
        district: v.district || '',
        postCode: v.post_code || '',
        publicationDate: v.publication_date || '',
        pdfUploadId: v.pdf_upload_id || '',
        address: {
          village: v.village || '',
          postOffice: v.voter_area || '',
          upazila: v.upazila || '',
          district: v.district || ''
        }
      }));

      // Client-side filters for mother/dob/gender (not handled by backend)
      const finalResults = mappedResults.filter((voter: VoterRecord) => {
        ...
        if (filters.gender === 'male') matchGender = voter.gender === 'পুরুষ';
        else if (filters.gender === 'female') matchGender = voter.gender === 'মহিলা';
  ...
  const handleAddVoter = async (newVoter: VoterRecord) => {
    try {
      const response = await fetch(`${API_BASE}/api/voters/register`, {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          serialNo: newVoter.serialNo,
          unionName: newVoter.unionName,
          wardNo: newVoter.wardNo,
          voterArea: newVoter.voterArea,
          voterAreaNo: newVoter.voterAreaNo,
          upazila: newVoter.upazila,
          district: newVoter.district,
          postCode: newVoter.postCode,
          publicationDate: newVoter.publicationDate,
          pdfUploadId: newVoter.pdfUploadId
        })
      });
